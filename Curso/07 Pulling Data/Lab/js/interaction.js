...
function generateData(){
	// Interacted statement
	var interactedVerbs = getInteractedStatememts();
	var intVerbsArr = ADL.XAPIWrapper.getStatements(interactedVerbs);

	// Started statement
	var startedVerbs = getStartedStatememts();
	var startedVerbsArr = ADL.XAPIWrapper.getStatements(startedVerbs);
	...
	if (startedVerbsArr) { 
		var startedCount = startedVerbsArr.statements.length;
		document.getElementById('startedCount').innerHTML = startedCount;
	}
}
...
function getStartedStatememts(){
	var myparams = ADL.XAPIWrapper.searchParams();
	myparams['since'] = timeSince.toISOString();  
	myparams['verb'] = 'https://w3id.org/xapi/dod-isd/verbs/started';  
	myparams['activity'] = 'http://learningdojo.net/xapi/simple_button';  
	myparams['agent'] = '{"mbox": "mailto:'+email+'"}';  
